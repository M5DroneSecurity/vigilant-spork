...
def data_parser(json_directory, json_filename, result_directory, include_directory):
    ...
    print("magic={} len={} inc={} com={} seq={} sys={} cid={} msg={} pay={} chk={}".format(len(magic), len(length),
                                                                                           len(incompat_flags),
                                                                                           len(compat_flags), len(seq),
                                                                                           len(sysid), len(compid),
                                                                                           len(msgid), len(payload),
                                                                                           len(checksum)))
    ...
    for m in msgid:
        message_name.append(message_decoder(str(m), include_directory, 'n'))
    for m in list(occur.keys()):
        occur_name.append(message_decoder(str(m), include_directory, 'n'))
        occur_def.append(message_decoder(str(m), include_directory, 'd'))
    ...
    clean = pd.DataFrame(np.column_stack([magic, length, incompat_flags, compat_flags, seq, sysid, compid, msgid,
                                          payload, checksum, payload_length, ip_src, ip_dst, frame_reltime,
                                          message_name]),
                         columns=['magic', 'length', 'incompat_flags', 'compat_flags', 'seq', 'sysid', 'compid',
                                  'msgid',
                                  'payload', 'checksum ', 'PAYLOAD_LENGTH', 'IP_SRC', 'IP_DST', 'FRAME_RELTIME',
                                  'MESSAGE'])
    ...
    '''
    with pd.ExcelWriter(result_directory + json_filename + '.xlsx', engine='xlsxwriter') as writer:
        ''' Bulk Data '''
